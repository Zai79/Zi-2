...
async function processQueue() {
  ...
  while (QUEUE.length > 0) {
    ...
    try {
      ...
      // return friendly fallback (do not throw)
      job.resolve({ ok: false, error: 'api_error' });
    }
    // small pause to avoid hammering the API under extreme load
    await new Promise(r => setTimeout(r, 120));
  }
  processing = false;
}

// ===== System Prompts (preserve your personality exactly, force Arabic only) =====
const PROMPTS = {
  miko: `
...
// ===== OpenRouter/ Together config (Qwen or Llama depending on key) =====
const API_URL = 'https://openrouter.ai/api/v1/chat/completions';
const MODEL = 'meta-llama/llama-3.1-405b-instruct'; // حسب إعدادك — يمكنك تغييره في المستقبل

// ===== call with retries & Arabic enforcement =====
async function callTogetherWithRetries(systemContent, userContent) {
  ...
  for (let attempt = 1; attempt <= MAX_TRIES; attempt++) {
    try {
      ...
      // enforce Arabic-only on the assistant reply: if reply contains English letters, request a safe Arabic rewrite
      if (/[A-Za-z]/.test(reply)) {
        // request a short Arabic-only rewrite
      ...
      lastErr = err;
      console.warn(`API attempt ${attempt} failed:`, err?.message || err);
      // small backoff
...
async function callTogether(systemContent, userContent) {
  ...
  const res = await fetch(API_URL, {
    ...
    // note: node-fetch doesn't support timeout option in v3; we'll handle externally if needed
  });
...
// ===== Helper: build system+user prompt per user & mode (session keeps last 15 entries) =====
function buildPrompts(userId, username, messageContent, isGirl) {
  if (!sessions.has(userId)) sessions.set(userId, []);
  const hist = sessions.get(userId);
  // store plain user messages (limit 15)
...
client.on('messageCreate', async (message) => {
  try {
    ...
    // build prompts and push to queue
    ...
    // wait up to 40s for a response, but queue will continue processing in background
    ...
    if (!result.ok) {
      if (result.error === 'timeout') {
        // friendly delayed reply: inform user but don't say "error"
        await message.reply('آسفة، الرد أخذ وقت أكثر من المعتاد — جرب مرّة ثانية لو تبي.');
        return;
      } else if (result.error === 'api_error') {
        await message.reply('آسفة، واجهت مشكلة داخلية الآن — حاول بعد لحظة.');
        return;
      } else {
        await message.reply('حصلت مشكلة بسيطة، حاول ثانية.');
        return;
      }
    }
    ...
    // ensure Arabic-only: final guard (if still contains ascii letters, ask model to rewrite, but avoid blocking user)
    if (/[A-Za-z]/.test(content)) {
      // short fallback Arabic response
      content = 'سأرد عليك بالعربية: ما قصّدت؟';
    }
...
// ===== Welcome (plain random text, not embed) =====
const WELCOMES = [
